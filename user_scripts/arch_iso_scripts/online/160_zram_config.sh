...
main() {
    ...
    # Note: [zram1] is intentionally omitted from base setup to preserve zero RAM table overhead.
    # It is delegated to user-space script 206 for on-demand hybrid backend resolution.
    cat >"$tmp_config" <<EOF
...
EOF
    ...
    log_success "ZRAM configuration generated successfully at ${CONFIG_FILE} (${ZRAM_SIZE_EXPR})"
}
