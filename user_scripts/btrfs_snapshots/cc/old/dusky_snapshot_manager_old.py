...
Design goals:
- No automatic Snapper snapshot creation on restore.
- True top-level subvolume creation.
- Correct hiding/protection of Snapper-owned subvolumes.
- Safer coordinated root/home handling.
- Safer restore/delete/backup primitives.
"""
...
def setup_logger() -> logging.Logger:
    ...
    try:
        ...
        syslog_handler.setFormatter(
            logging.Formatter("dusky-master[%(process)d]: %(levelname)s - %(message)s")
        )
...
# =============================================================================
# GLOBAL CONSTANTS / SAFETY HELPERS
# =============================================================================
DUSKY_RUN_DIR = Path("/run/dusky")
DUSKY_LOCK_FILE = DUSKY_RUN_DIR / "dusky.lock"
...
# Physical top-level snapshot roots used by the isolation script.
...
def dusky_lock():
    """
    Global advisory lock to prevent concurrent destructive Dusky operations.
    """
    try:
        DUSKY_RUN_DIR.mkdir(parents=True, exist_ok=True)
        fd = os.open(DUSKY_LOCK_FILE, os.O_CREAT | os.O_RDWR, 0o600)
    except OSError as exc:
        fail(f"[!] Failed to acquire Dusky lock: {exc}")
...
def critical_signals():
    """
    Temporarily ignore Ctrl-C / SIGTERM during destructive rename windows.
    """
    try:
...
def sync_btrfs(path: Path | str) -> None:
    """
    Best-effort Btrfs sync after mutations.
    """
    run_cmd(["btrfs", "filesystem", "sync", str(path)], check=False)


def pause_prompt(message: str = "\n\033[1;38;5;114mPress Enter to return...\033[0m") -> None:
    try:
        input(message)
...
def run_passthrough(cmd: list[str]) -> int:
    try:
        return subprocess.run(cmd, env=SUBPROCESS_ENV).returncode
    except OSError as exc:
        fail(f"[!] Command execution failed: {shlex.join(cmd)}\n{exc}")
    return 1
...
def _subvol_from_options(options: str) -> str | None:
    match = re.search(r"(?:^|,)subvol=([^,]+)(?:,|$)", options.strip())
    if not match:
        return None
    return match.group(1).strip().strip('"').lstrip("/")


def get_fs_uuid(mountpoint: str) -> str:
    res = run_cmd(["findmnt", "-n", "-e", "-o", "UUID", "-M", mountpoint], check=False)
    if res.returncode == 0:
        fs_uuid = _first_line(res.stdout)
        if fs_uuid and fs_uuid != "-":
            return fs_uuid

    res = run_cmd(["findmnt", "-n", "-e", "-o", "SOURCE", "-M", mountpoint], check=False)
    if res.returncode != 0:
        return ""

    source = _first_line(res.stdout)
    ...
    if source.startswith("/dev/"):
        ...
        blk = run_cmd(["blkid", "-s", "UUID", "-o", "value", dev], check=False)
        if blk.returncode == 0:
            return _first_line(blk.stdout)
...
def get_btrfs_device(mountpoint: str) -> str:
    res = run_cmd(["findmnt", "-n", "-e", "-o", "SOURCE", "-M", mountpoint], check=False)
    source = _first_line(res.stdout) if res.returncode == 0 else ""
    source = re.sub(r"\[.*?\]$", "", source).strip()
    ...
    fs_uuid = get_fs_uuid(mountpoint)
    if fs_uuid:
        return f"UUID={fs_uuid}"

    fail(
        f"[!] Fatal: Could not resolve physical block device or UUID for {mountpoint}. "
        f"Found: {source or '<nothing>'}"
    )


def get_active_subvol(mountpoint: str, fatal: bool = True) -> str | None:
    # 1. Live mount options
    res = run_cmd(["findmnt", "-n", "-o", "OPTIONS", "-M", mountpoint], check=False)
    if res.returncode == 0:
        for line in res.stdout.splitlines():
            sv = _subvol_from_options(line)
            if sv is not None and sv != "":
                return sv

    # 2. Live btrfs metadata
    res = run_cmd(["btrfs", "subvolume", "show", mountpoint], check=False)
    if res.returncode == 0:
        match = re.search(r"^[ \t]*Path:[ \t]*(.+)$", res.stdout, re.MULTILINE)
        if match:
    ...
    # 3. fstab fallback
    res = run_cmd(["findmnt", "--fstab", "-n", "-o", "OPTIONS", "-M", mountpoint], check=False)
    if res.returncode == 0:
        for line in res.stdout.splitlines():
            sv = _subvol_from_options(line)
            if sv is not None and sv != "":
    ...
    if fatal:
        fail(
            f"[!] Fatal: Could not determine active Btrfs subvolume path for {mountpoint}. "
            "No usable subvol= option or live Path was found."
        )
    return None


def get_target_mount_from_snapper_config(config: str) -> str:
    cfg_file = Path(f"/etc/snapper/configs/{config}")
    if cfg_file.is_file():
        try:
            content = cfg_file.read_text(errors="ignore")
            match = re.search(r'^SUBVOLUME=["\']?([^"\'\n]+)["\']?', content, re.MULTILINE)
            if match:
                target_mnt = os.path.normpath(match.group(1).strip())
                if target_mnt:
                    return target_mnt
        except Exception:
            pass

    result = run_cmd(["snapper", "-c", config, "get-config"], check=False)
    if result.returncode == 0:
        for line in result.stdout.splitlines():
            sanitized_line = line.replace("│", "|")
            key, sep, value = sanitized_line.partition("|")
            if sep and key.strip() == "SUBVOLUME":
                target_mnt = os.path.normpath(value.strip())
                if target_mnt:
                    return target_mnt
                break
...
def get_snapper_configs() -> list[dict[str, str]]:
    ...
    if config_dir.is_dir():
        for cfg_file in config_dir.iterdir():
            ...
            try:
                ...
                match = re.search(r'^SUBVOLUME=["\']?([^"\'\n]+)["\']?', content, re.MULTILINE)
                if match:
                    sub = os.path.normpath(match.group(1).strip())
            except Exception:
                pass
    ...
    if res.returncode == 0:
        ...
        for row in reader:
            if len(row) >= 2:
                cfg = row[0].strip()
                sub = os.path.normpath(row[1].strip())
                if cfg:
                    configs.append({"config": cfg, "subvolume": sub})
...
def get_btrfs_mounts() -> list[BtrfsMount]:
    res = run_cmd(
        ["findmnt", "-t", "btrfs", "-J", "-e", "-o", "SOURCE,TARGET,FSTYPE,OPTIONS,UUID"],
        check=False,
    )
    ...
    try:
        return json.loads(res.stdout).get("filesystems", [])
    except json.JSONDecodeError:
        return []


def get_fstab_protected_subvols() -> set[str]:
    ...
    try:
        for line in Path("/etc/fstab").read_text(errors="ignore").splitlines():
            ...
            sv = _subvol_from_options(fields[3])
            if sv:
...
def get_snapper_snapshot_roots() -> set[str]:
    roots = set(DEFAULT_SNAPSHOT_ROOTS)

    for cfg in get_snapper_configs():
        base = cfg.get("subvolume", "/").strip() or "/"
        snaps_mnt = "/.snapshots" if base == "/" else f"{base.rstrip('/')}/.snapshots"
        sv = get_active_subvol(snaps_mnt, fatal=False)
        if sv:
            roots.add(sv.strip("/"))

    for m in get_btrfs_mounts():
        target = str(m.get("target", "")).rstrip("/")
        options = str(m.get("options", ""))

        if target == "/.snapshots" or target.endswith("/.snapshots"):
            sv = _subvol_from_options(options)
            if sv:
                roots.add(sv.strip("/"))

    return {r.strip("/") for r in roots if r and r.strip("/")}


def is_hidden_subvolume_path(sv_path: str, snapshot_roots: set[str]) -> bool:
    p = sv_path.strip("/")

    if any(token in p for token in TRANSIENT_TOKENS):
        return True

    if p == ".snapshots" or p.startswith(".snapshots/"):
        return True
...
# =============================================================================
# TOPOLOGY, NOCOW & EXTERNAL BACKUP
# =============================================================================
@contextmanager
def mount_top_level(device: str, quiet: bool = False, read_only: bool = False) -> Iterator[Path]:
    base_dir = Path("/run/dusky/mnt")
    base_dir.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="top_", dir=str(base_dir), ignore_cleanup_errors=True) as tmpdir:
        mnt_point = Path(tmpdir)

        if not quiet:
            print(
                f"\033[1;38;5;81m[*] Mounting top-level tree (subvolid=5) for {device}...\033[0m",
                file=sys.stderr,
            )
        ...
        res = run_cmd(["mount", "-t", "btrfs", "-o", opts, device, str(mnt_point)], check=False)

        if res.returncode != 0 and not read_only:
            res = run_cmd(["mount", "-t", "btrfs", "-o", "subvolid=5,ro", device, str(mnt_point)], check=False)
        ...
        try:
            ...
            if not unmounted:
                message = error_text(result) if result is not None else "unknown umount failure"
                log_msg = (
                    f"Failed to cleanly unmount {mnt_point} after 3 attempts: {message}. "
                    "Filesystem may be busy."
                )
                ...
                if quiet:
                    ...
                    if active_exception is None:
                        fail(
                            f"[!] Command failed: umount {mnt_point}\n{message}",
...
def get_all_subvolumes() -> list[SubvolMeta]:
    ...
    snapshot_roots = get_snapper_snapshot_roots()

    for m in mounts:
        raw_dev = str(m.get("source", "")).strip()
        if not raw_dev:
            continue

        fs_uuid = str(m.get("uuid", "")).strip()
        clean_source = re.sub(r"\[.*?\]$", "", raw_dev).strip()

        if clean_source.startswith("UUID="):
            fs_uuid = clean_source.split("=", 1)[1].strip() or fs_uuid
            dev = f"UUID={fs_uuid}"
        elif clean_source.startswith("/dev/"):
            dev = os.path.realpath(clean_source)
        else:
            dev = clean_source
        ...
        target_hint = str(m.get("target", "/"))

        try:
            with mount_top_level(dev, quiet=True, read_only=True) as top_mnt:
                ...
                for line in res.stdout.splitlines():
                    ...
                    if is_hidden_subvolume_path(sv_path, snapshot_roots):
                        continue
...
def validate_top_level_name(name: str) -> str:
    name = name.strip().lstrip("/")

    if not name:
        fail("[!] Fatal: Subvolume name cannot be empty.")
    if name in (".", ".."):
        fail("[!] Fatal: Invalid subvolume name.")
    if "/" in name:
        fail("[!] Fatal: Top-level mode does not allow '/'. Use a direct top-level name like @data.")
    if any(ch.isspace() for ch in name):
        fail("[!] Fatal: Subvolume names with spaces are intentionally disallowed for boot/fstab/systemd safety.")
    if name.startswith("."):
        fail("[!] Fatal: Subvolume names starting with '.' are disallowed.")

    return name


def create_top_level_subvolume(device: str, name: str, disable_cow: bool) -> None:
    name = validate_top_level_name(name)

    with dusky_lock(), mount_top_level(device) as top_mnt:
        full_path = top_mnt / name

        if full_path.exists():
            fail(f"[!] Target path already exists at top level: {full_path}")

        print(f"\033[1;38;5;81m[*] Creating TOP-LEVEL BTRFS subvolume: {name}\033[0m")
        run_cmd(["btrfs", "subvolume", "create", str(full_path)])

        if disable_cow:
            print("\033[1;38;5;220m[*] Applying NOCOW attribute (chattr +C)...\033[0m")
            run_cmd(["chattr", "+C", str(full_path)])

            lsattr = run_cmd(["lsattr", "-d", str(full_path)], check=False)
            if lsattr.returncode == 0 and "C" not in lsattr.stdout:
                print(
                    "\033[1;38;5;220m[!] Warning: NOCOW flag could not be verified. "
                    "It may still be set, but verify manually.\033[0m"
                )
    ...
    print(
        "\033[1;38;5;81m[*] If you want to mount it, use:\n"
        f"    mount -o subvol=/{name} <device-or-uuid> /your/mountpoint\033[0m"
    )
...
def backup_snapshot_to_external(src_dev: str, src_rel_path: str, external_dest: str) -> None:
    ...
    dest_uuid = get_fs_uuid(str(dest_path))
    if not dest_uuid:
        fail(f"[!] Could not resolve Btrfs filesystem UUID for destination: {dest_path}")

    print(f"\033[1;38;5;81m[*] Preparing backup from {src_rel_path} to {dest_path}...\033[0m")

    with dusky_lock(), mount_top_level(src_dev) as src_mnt:
        src_fs_uuid = get_fs_uuid(str(src_mnt))
        if src_fs_uuid and dest_uuid and src_fs_uuid == dest_uuid:
            print(
                "\033[1;38;5;220m[!] Warning: destination is on the same Btrfs filesystem as source.\n"
                "[!] This is not a true external backup.\033[0m"
            )

        print("\033[38;5;246m[*] Sweeping for orphaned ephemeral backups...\033[0m")
        ...
        if not is_ro:
            print(
                "\033[1;38;5;220m[*] Source is writable. Creating ephemeral read-only snapshot "
                "for a consistent stream...\033[0m"
            )
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            ephemeral_snap = src_mnt / f".tmp_send_{src_path.name or 'root'}_{timestamp}"
            run_cmd_raise(["btrfs", "subvolume", "snapshot", "-r", str(src_path), str(ephemeral_snap)])
            src_path = ephemeral_snap

        try:
            try:
                staging_dir = Path(tempfile.mkdtemp(dir=str(dest_path), prefix=".btrfs_recv_"))
            except OSError as exc:
                fail(f"[!] Failed to create staging directory under {dest_path}: {exc}")

            try:
                print(f"\033[38;5;246mExecuting stream: btrfs send {src_path} | btrfs receive {staging_dir}\033[0m")

                with subprocess.Popen(
                    ["btrfs", "send", str(src_path)],
                    stdout=subprocess.PIPE,
                    env=SUBPROCESS_ENV,
                ) as send_proc:
                    recv_proc = subprocess.Popen(
                        ["btrfs", "receive", str(staging_dir)],
                ...
                if recv_proc.returncode != 0:
                    fail(f"[!] btrfs receive failed:\n{recv_err.strip()}")

                if send_rc not in (0, -13, 141):
                    fail(f"[!] btrfs send failed with return code {send_rc}")
                ...
                if original_name == "snapshot":
                    original_name = f"snapshot_{Path(src_rel_path).parent.name}"

                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                final_dest = dest_path / f"backup_snap_{original_name}_{timestamp}"

                if final_dest.exists():
                    fail(f"[!] Fatal: Target backup already exists: {final_dest}")

                src_item.rename(final_dest)
                sync_btrfs(dest_path)

                print("\033[1;38;5;114m[+] Backup successful!\033[0m")
                print(f"\033[1;38;5;114m[+] Backup subvolume created at: {final_dest}\033[0m")
            ...
            if ephemeral_snap and ephemeral_snap.exists():
                print("\033[38;5;246m[*] Cleaning up ephemeral snapshot...\033[0m")
...
def resolve_restore_spec(config: str, snap_id: str) -> RestoreSpec:
    ...
    fs_uuid = get_fs_uuid(target_mnt)

    if not fs_uuid and device.startswith("UUID="):
        fs_uuid = device.split("=", 1)[1].strip()

    if not fs_uuid:
        fail(f"[!] Fatal: Could not determine filesystem UUID for {target_mnt}.")
...
def ensure_no_nested_subvolumes(plan: PreparedRestore) -> None:
    ...
    if result.returncode != 0:
        fail(
            f"[!] Fatal: Failed to inspect nested subvolumes inside '{plan.spec.active_subvol}' "
            f"for config '{plan.spec.config}'.\n{error_text(result)}"
        )

    nested_output = result.stdout.strip()
    if nested_output:
        fail(
            f"\n[!] CRITICAL HALT: Nested subvolumes detected physically inside '{plan.spec.active_subvol}' "
            f"for config '{plan.spec.config}'!\n"
            f"Offending subvolumes:\n{nested_output}\n"
            "[!] An atomic rollback would trap these inside the subvolume slated for deletion.\n"
            "[!] Flatten your Btrfs topology first."
        )
    ...
    for plan in reversed(plans):
        if plan.activated and plan.target_path.exists() and not plan.staging_path.exists():
            try:
                ...
                rollback_errors.append(f"{plan.spec.config}: failed to move restored subvolume out of the way: {exc}")
    ...
    for plan in reversed(plans):
        if plan.active_moved and plan.temp_delete_path.exists() and not plan.target_path.exists():
            try:
                ...
                rollback_errors.append(f"{plan.spec.config}: failed to restore original active subvolume: {exc}")
        ...
        if plan.staging_path.exists():
            ...
            if result.returncode != 0:
                rollback_errors.append(
                    f"{plan.spec.config}: failed to delete staging subvolume '{plan.staging_path.name}': {error_text(result)}"
                )
...
def apply_prepared_restores(plans: list[PreparedRestore], top_mnt: Path | None = None) -> None:
    ...
    for plan in plans:
        ...
        if not plan.target_path.is_dir():
            fail(f"[!] Fatal: Active subvolume path does not exist for config '{plan.spec.config}': {plan.target_path}")
        if plan.temp_delete_path.exists():
            fail(f"[!] Fatal: Deletion path already exists for config '{plan.spec.config}': {plan.temp_delete_path}")
        if plan.staging_path.exists():
            fail(f"[!] Fatal: Staging path already exists for config '{plan.spec.config}': {plan.staging_path}")

        ensure_no_nested_subvolumes(plan)

    try:
        # Phase 1: create staged writable snapshots
        for plan in plans:
            print(
        ...
        # Phase 2 + 3: destructive rename window
        with critical_signals():
            for plan in plans:
            ...
            for plan in plans:
                print(
                    f"\033[1;38;5;81m[*] Activating restored snapshot for '{plan.spec.config}' "
                    f"as {plan.target_path.name}...\033[0m"
                )
                plan.staging_path.rename(plan.target_path)
                plan.activated = True

        if top_mnt is not None:
            sync_btrfs(top_mnt)

        # Phase 4: delete or schedule deletion of old active subvolume
        for plan in plans:
            ...
            if not is_active_mount:
                ...
                print(
                    f"\033[1;38;5;81m[*] Deferring deletion of active system state for '{plan.spec.config}' "
                    "to next boot...\033[0m"
                )
            ...
            if not is_active_mount:
                print(
                    "\033[1;38;5;220m[!] Warning: Immediate deletion failed. Scheduling boot-time cleanup...\033[0m",
                    file=sys.stderr,
                )

            try:
                fs_uuid = plan.spec.fs_uuid
                if not fs_uuid:
                    print(
                        f"\033[1;38;5;196m[!] Error: Could not determine UUID for {plan.spec.target_mnt}. "
                        "Manual deletion required.\033[0m",
                        file=sys.stderr,
                    )
                    continue

                subvol_name = plan.temp_delete_path.name
                safe_hash = hashlib.sha1(subvol_name.encode("utf-8")).hexdigest()[:16]
                service_name = f"dusky-cleanup-{safe_hash}.service"

                esc_res = run_cmd(["systemd-escape", "--path", subvol_name], check=False)
                escaped_subvol = esc_res.stdout.strip() if esc_res.returncode == 0 else subvol_name.replace(" ", "_")

                root_plan = next((p for p in plans if p.spec.target_mnt == "/"), None)
                if root_plan:
                    systemd_dir = root_plan.target_path / "etc" / "systemd" / "system"
                    dusky_lib_dir = root_plan.target_path / "var" / "lib" / "dusky"
                else:
                    systemd_dir = Path("/etc/systemd/system")
                    dusky_lib_dir = Path("/var/lib/dusky")

                service_path = systemd_dir / service_name
                service_path.parent.mkdir(parents=True, exist_ok=True)

                staged_script = dusky_lib_dir / "dusky_snapshot_manager.py"
                try:
                    dusky_lib_dir.mkdir(parents=True, exist_ok=True)
                    if SCRIPT_PATH.is_file():
                        shutil.copy2(SCRIPT_PATH, staged_script)
                        staged_script.chmod(0o755)
                        live_dusky_lib = Path("/var/lib/dusky")
                        live_dusky_lib.mkdir(parents=True, exist_ok=True)
                        shutil.copy2(SCRIPT_PATH, live_dusky_lib / "dusky_snapshot_manager.py")
                        (live_dusky_lib / "dusky_snapshot_manager.py").chmod(0o755)
                except Exception as e:
                    LOG.warning(f"Could not stage script to /var/lib/dusky: {e}")

                exe = sys.executable or "/usr/bin/python3"
                script_path = "/var/lib/dusky/dusky_snapshot_manager.py"
...
[Service]
Type=oneshot
ExecStart="{exe}" "{script_path}" --cleanup-subvol {fs_uuid} {escaped_subvol}
...
"""
                ...
                print(
                    f"\033[1;38;5;114m[+] Scheduled one-shot systemd service '{service_name}' "
                    "to remove the old subvolume on next boot.\033[0m"
                )
            except Exception as e:
                print(
                    f"\033[1;38;5;196m[!] Failed to schedule boot cleanup: {e}\n"
                    "[!] Manual deletion required.\033[0m",
                    file=sys.stderr,
                )
...
def parse_snapshot_datetime(raw_value: object) -> datetime | None:
    ...
    if len(tokens) >= 6:
        try:
            cleaned = " ".join(tokens)
            return datetime.strptime(cleaned, "%a %b %d %H:%M:%S %Y").astimezone()
        except ValueError:
            pass
...
def snapshot_records_to_gui(records: list[dict[str, object]]) -> list[dict[str, str]]:
    ...
    for record in records:
        ...
        userdata_value = first_present(record, "userdata", "user_data")
        if isinstance(userdata_value, dict):
            userdata_dict = {str(k).strip(): str(v).strip() for k, v in userdata_value.items()}
            userdata = ",".join(f"{k}={v}" for k, v in userdata_dict.items())
        else:
            userdata = "" if userdata_value is None else str(userdata_value)
            userdata_dict = parse_userdata(userdata)
...
def load_all_snapper_data() -> list[dict[str, str]]:
    ...
    for cfg_info in configs:
        ...
        snaps_mnt_mounted = Path(snaps_mnt).is_mount()
        ...
        for s in gui_snaps:
            ...
            s["dead"] = bool(snaps_mnt_mounted and not Path(s["location"]).exists())

            if s["dead"] and not s.get("description", "").startswith("[DEAD]"):
                s["description"] = "[DEAD] " + s.get("description", "")
...
def find_coordinated_pair(
    ...
    """
    Find the matching home snapshot for a root snapshot.

    Priority:
      1. explicit dusky_pair userdata
      2. explicit root ID hint + exact/raw matching
      3. exact raw_date match
      4. same description + nearest timestamp within 15 minutes
    """
    ...
    if root_id_hint is not None:
        hint_matches = [s["id"] for s in root_snaps if s["id"] == str(root_id_hint)]
        if len(hint_matches) == 1:
            root_id = hint_matches[0]
    ...
    if not isinstance(target_userdata, dict):
        target_userdata = {}

    pair_id = target_userdata.get("dusky_pair")
    if pair_id:
        pair_home = [
            s["id"]
            for s in home_snaps
            if s.get("userdata_dict", {}).get("dusky_pair") == pair_id
        ]

        if len(pair_home) == 1:
            return root_id, pair_home[0]
        if len(pair_home) > 1:
            raise RuntimeError("[!] Fatal: Multiple Home snapshots matched the same dusky_pair userdata.")

    exact_home = [s for s in home_snaps if s.get("raw_date") == target_date]
    if target_desc:
        exact_home_desc = [s for s in exact_home if s.get("description") == target_desc]
        if exact_home_desc:
            exact_home = exact_home_desc

    if len(exact_home) == 1:
        return root_id, exact_home[0]["id"]

    candidates = home_snaps
    if target_desc:
        desc_matches = [s for s in home_snaps if s.get("description") == target_desc]
        if desc_matches:
            candidates = desc_matches

    if len(candidates) == 1:
        return root_id, candidates[0]["id"]

    target_dt = parse_snapshot_datetime(target_date)
    if not target_dt:
        raise RuntimeError("[!] Fatal: Date parsing failed for target date. Cannot perform safe fallback.")
    ...
    if best_id is not None and best_diff <= 900:
        return root_id, best_id

    if best_id is not None:
        raise RuntimeError(
            f"[!] Fatal: Closest Home snapshot (ID {best_id}) is {best_diff:.1f}s away. "
            "Exceeds strict 900s safety threshold."
        )
...
def activate_nonroot_restore(target_mnt: str) -> None:
    ...
    if run_cmd(["umount", target_mnt], check=False).returncode != 0:
        print(
            ...
            "[\033[1;38;5;196m!\033[1;38;5;220m] WARNING: Any changes made to this mount right now may be lost on reboot.\n"
    ...
    if run_cmd(["mount", target_mnt], check=False).returncode != 0:
        fail(
            ...
            f"[!] Your {target_mnt} directory is currently unmounted. Resolve manually before rebooting."
        )
...
def handle_create_pair(config1: str, config2: str, description: str) -> None:
    print(
        f"\033[1;38;5;81m[*] Creating coordinated snapshots for '{config1}' and '{config2}': {description}\033[0m"
    )
    ...
    with dusky_lock():
        run_cmd(
            [
                ...
                description,
                "--userdata",
                f"dusky_pair={pair_id},dusky_role={config1}",
            ]
        )
        run_cmd(
            [
                ...
                description,
                "--userdata",
                f"dusky_pair={pair_id},dusky_role={config2}",
            ]
        )
...
def handle_delete(config: str, snap_id: str) -> None:
    ...
    with dusky_lock():
        result = run_cmd(["snapper", "-c", config, "delete", snap_id], check=False)
        ...
        target_mnt = get_target_mount_from_snapper_config(config)
        snapshots_mnt = "/.snapshots" if target_mnt == "/" else f"{target_mnt}/.snapshots"
        snap_dir = Path(snapshots_mnt) / snap_id
        snap_subvol = snap_dir / "snapshot"

        if snap_dir.is_dir() and not snap_subvol.exists():
            shutil.rmtree(snap_dir, ignore_errors=True)
            if not snap_dir.exists():
                print(f"\033[1;38;5;220m[+] Dead snapshot metadata for ID {snap_id} purged safely.\033[0m")
                return

        fail(
            f"[!] Failed to delete snapshot ID {snap_id} for '{config}'.\n"
            f"{error_text(result)}"
        )
...
def handle_restore(config: str, snap_id: str, no_remount: bool = False) -> None:
    ...
    # IMPORTANT:
    # No automatic Snapper snapshot is created here.
    with dusky_lock(), mount_top_level(spec.device) as top_mnt:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        plan = prepare_restore(spec, top_mnt, timestamp)
        apply_prepared_restores([plan], top_mnt)
    ...
    if no_remount:
        print(
            f"\033[1;38;5;220m[!] {spec.target_mnt} was restored on disk without live remount.\n"
...
def handle_restore_pair(config1: str, snap_id1: str, config2: str, snap_id2: str) -> None:
    ...
    if spec1.active_subvol == spec2.active_subvol:
        fail("[!] Fatal: Coordinated restore configs resolve to the same subvolume path.")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")

    specs_by_device: dict[str, list[RestoreSpec]] = {}
    for spec in (spec1, spec2):
        specs_by_device.setdefault(spec.device, []).append(spec)

    with dusky_lock():
        for device, spec_list in specs_by_device.items():
            with mount_top_level(device) as top_mnt:
                plans = [prepare_restore(spec, top_mnt, timestamp) for spec in spec_list]
                apply_prepared_restores(plans, top_mnt)

    print("\n\033[1;38;5;114m[+] Coordinated restoration complete.\033[0m")

    has_root = spec1.target_mnt == "/" or spec2.target_mnt == "/"
    if has_root:
...
def handle_cleanup_subvol(fs_uuid: str, escaped_path: str) -> None:
    """
    Internal handler used by generated oneshot systemd cleanup units.
    """
    res = run_cmd(["systemd-escape", "--unescape", "--path", escaped_path], check=False)
    subvol_path = res.stdout.strip() if res.returncode == 0 else escaped_path

    device = f"UUID={fs_uuid}"

    with dusky_lock(), mount_top_level(device, quiet=True) as top_mnt:
        target = top_mnt / subvol_path.lstrip("/")

        if not target.exists():
            return

        if is_mountpoint(str(target)):
            print(f"[!] Refusing to delete mounted subvolume: {subvol_path}", file=sys.stderr)
            return

        run_cmd(["btrfs", "subvolume", "delete", str(target)], check=False)
        sync_btrfs(top_mnt)
...
def handle_tui_preview(view: str, line: str, show_diff: bool = False) -> None:
    try:
        ...
        if is_subvol:
            shortcuts.extend(
                [
                    "\033[1;38;5;114m[CTRL-N]\033[0m  \033[38;5;253m󰐕 Create Top-Level Subvol\033[0m",
        ...
        if is_subvol:
            ...
            print("\033[1;38;5;213m󰋊 SUBVOLUME METADATA\033[0m")
            print("\033[38;5;238m" + "─" * 48 + "\033[0m")
            ...
            print("\033[1;38;5;81m󰆑 SNAPSHOT DETAILS\033[0m")
            print("\033[38;5;238m" + "─" * 48 + "\033[0m")
            ...
            if pre:
                print(f" \033[1;38;5;246mPre-ID \033[0m │ \033[38;5;216m{pre}\033[0m")
            ...
            if cln and cln.lower() != "none":
                print(f" \033[1;38;5;246mCleanup\033[0m │ \033[38;5;216m{cln}\033[0m")

            loc = meta.get("location")
            if loc:
                print(f" \033[1;38;5;246mPath   \033[0m │ \033[38;5;114m{loc}\033[0m")

            if meta.get("dead"):
                print(" \033[1;38;5;196mState  \033[0m │ \033[1;38;5;196mDEAD / MISSING\033[0m")

            userdata = meta.get("userdata")
            if userdata:
                print(f" \033[1;38;5;246mUserdata\033[0m │ \033[38;5;246m{userdata}\033[0m")

            print(f" \033[1;38;5;246mDesc   \033[0m │ \033[38;5;253m{meta.get('description', 'N/A')}\033[0m\n")

        if show_diff:
            if is_subvol:
                print("\033[1;38;5;246m[!] Diff mode not applicable for raw subvolumes.\033[0m")
            else:
                print("\033[1;38;5;114m󰏫 FILES CHANGED IF RESTORED\033[0m \033[3;38;5;246m(vs Current System)\033[0m")
                print("\033[38;5;238m" + "─" * 48 + "\033[0m")
                ...
                if view in ("home", "root", "global"):
                    ...
                    print()
                    try:
                        _, h_id = find_coordinated_pair(
                            meta.get("raw_date", ""),
                            meta.get("description", ""),
                            meta.get("userdata_dict", {}),
                            str(meta.get("id")),
                        )
                        run_diff("home", h_id)
                    except RuntimeError:
                        print(
                            f"\033[1;38;5;203m▶ System Profile: home\033[0m\n"
                            f"\033[3;38;5;196mFailed to locate paired snapshot.\033[0m"
                        )
        else:
            print("\033[1;38;5;246m[!] File changes hidden for performance.\033[0m")
            print("\033[1;38;5;246mPress \033[1;38;5;220m<Ctrl+V>\033[1;38;5;246m to generate file change list.\033[0m")
            print("\033[1;38;5;246mPress \033[1;38;5;220m<Ctrl+P>\033[1;38;5;246m to hide and restore fast scrolling.\033[0m")
...
def launch_tui() -> None:
    if not shutil.which("fzf"):
        fail("[!] Fatal: 'fzf' is required. Install via: pacman -S fzf")
    ...
    while True:
        ...
        storage_hdr = (
            f" \033[1;38;5;81m󰋊 BTRFS STORAGE:\033[0m "
            f"\033[38;5;253m{total / gb:.1f} GB Total\033[0m "
            f"\033[38;5;238m|\033[0m "
            f"\033[38;5;203m{used / gb:.1f} GB Used\033[0m "
            f"\033[38;5;238m|\033[0m "
            f"\033[38;5;114m{free / gb:.1f} GB Free\033[0m "
        )
        ...
        if current_view == "subvolumes":
            table_hdr = (
                f"\033[1;38;5;242m{'ID':>4}\033[0m {c_sep} "
                f"\033[1;38;5;242m{'DEVICE HINT':<15}\033[0m {c_sep} "
                f"\033[1;38;5;242mBTRFS PATH\033[0m"
            )
            ...
            table_hdr = (
                f"\033[1;38;5;242m{'CFG':<8}\033[0m {c_sep} "
                f"\033[1;38;5;242m{'ID':>4}\033[0m {c_sep} "
                f"\033[1;38;5;242m{'AGE':<10}\033[0m {c_sep} "
                f"\033[1;38;5;242m{'DATE':<18}\033[0m {c_sep} "
                f"\033[1;38;5;242mDESCRIPTION\033[0m"
            )
            lines_for_fzf.extend([mode_hdr, hr, table_hdr])
            ...
            if snaps:
                for s in sorted(
                    snaps,
                    key=lambda x: (x.get("config", ""), -int(x.get("id", "0") or 0)),
                ):
                    ...
                    meta = {
                        ...
                        "userdata_dict": s.get("userdata_dict", {}),
                ...
                dummy_vis = (
                    f"\033[38;5;246m{'Empty':<8}\033[0m {c_sep} "
                    f"{'':>4} {c_sep} {'':<10} {c_sep} "
                    f"{'No snapshots':<18} {c_sep}"
                )
                lines_for_fzf.append(f"{dummy_vis}\x1f{json.dumps({'empty': True})}")
            ...
            hdr_desc = "\033[1;38;5;242mDESCRIPTION\033[0m"
        ...
        fzf_cmd = [
            ...
            f"ctrl-p:change-preview({preview_cmd})+change-prompt( :: Action ❯ ),"
            f"ctrl-v:change-preview({preview_diff_cmd})+change-prompt( :: Diff Mode ON ❯ ),"
        ...
        if current_view == "coordinated":
            ...
            for m_data in selected_metas:
                if m_data.get("dead"):
                    print("\n\033[1;38;5;196m[!] One selected snapshot is dead and cannot be paired/restored.\033[0m")
                    has_error = True
                    continue
                ...
                try:
                    r_id, h_id = find_coordinated_pair(
                        m_data.get("raw_date", ""),
                        m_data.get("description", ""),
                        m_data.get("userdata_dict", {}),
                        sid,
                    )
            ...
            if key_pressed == "enter":
                ...
                print(
                    "\n\033[1;38;5;196m[!] Backup is not supported in Coordinated mode. "
                    "Switch to Global or Subvolumes tab.\033[0m"
                )
                pause_prompt()
            ...
            if key_pressed == "enter":
                ...
                if meta.get("dead"):
                    print("\n\033[1;38;5;196m[!] This snapshot is dead: its physical subvolume is missing.\033[0m")
                    pause_prompt()
                    continue
                ...
                if confirm_prompt(f"Permanently delete {len(selected_metas)} snapshot(s)?"):
                    for m in selected_metas:
                        handle_delete(m["config"], str(m["id"]))
                ...
                if meta.get("dead"):
                    print("\n\033[1;38;5;196m[!] Cannot back up a dead snapshot.\033[0m")
                    pause_prompt()
                    continue
                ...
                if loc:
                    ...
                    try:
                        ...
                        if dest:
                            backup_snapshot_to_external(dev, sv_rel, dest)
                    except (KeyboardInterrupt, EOFError):
                        pass
            ...
            match key_pressed:
                case "ctrl-n":
                    print("\n\033[1;38;5;213m[*] ACTION: CREATE NEW TOP-LEVEL SUBVOLUME\033[0m")
                    try:
                        name = input("\033[1;38;5;220m[*] New top-level subvolume name (e.g., @data): \033[0m").strip()
                        if name:
                            disable_cow = confirm_prompt("Disable Copy-On-Write (NOCOW / chattr +C)?")
                            create_top_level_subvolume(dev, name, disable_cow)
                    except (KeyboardInterrupt, EOFError):
                        pass
                ...
                case "ctrl-s":
                    ...
                    try:
                        default_snap = (
                            f"@snapshots/{sv_path.lstrip('/@').replace('/', '_')}_"
                            f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
                        )
                        dest_rel = input(
                            "\033[1;38;5;220m[*] Destination path "
                            f"(Relative to BTRFS root. Enter for default: {default_snap}): \033[0m"
                        ).strip()
                        ...
                        if dest_rel:
                            ...
                            with dusky_lock(), mount_top_level(dev) as top_mnt:
                                src_abs = top_mnt / sv_path.lstrip("/")
                                dest_abs_path = top_mnt / dest_rel.lstrip("/")
                                ...
                                run_cmd(cmd + [str(src_abs), str(dest_abs_path)])
                                sync_btrfs(top_mnt)
                ...
                case "ctrl-g":
                    ...
                    try:
                        print(
                            "\033[1;38;5;220m[!] Note: The target subvolume MUST be mounted live "
                            "on your system for Snapper to initialize.\033[0m"
                        )
                ...
                case "ctrl-b":
                    ...
                    try:
                        ...
                        if dest:
                            backup_snapshot_to_external(dev, sv_path.lstrip("/"), dest)
                    except (KeyboardInterrupt, EOFError):
                        pass
                ...
                case "delete" | "ctrl-d":
                    ...
                    for mnt in get_btrfs_mounts():
                        ...
                        if match:
                            ...
                            show_cmd = run_cmd(["btrfs", "subvolume", "show", str(target)], check=False)
                    ...
                    protected_subvols.update(get_snapper_snapshot_roots())
                    protected_subvols.update(get_fstab_protected_subvols())
                    ...
                    for m in selected_metas:
                        ...
                        if p_clean in protected_subvols:
                            print(
                                f"\033[1;38;5;196m[!] SYSTEM GUARDRAIL ACTIVATED: "
                                f"Refusing to delete protected subvolume -> {m['path']}\033[0m"
                            )
                        else:
...
def main() -> None:
    ...
    if args.cleanup_subvol is not None:
        handle_cleanup_subvol(args.cleanup_subvol[0], args.cleanup_subvol[1])
        return
    ...
    action_present = any(
        [
            ...
            args.restore_pair is not None,
            args.delete_pair is not None,
            args.sync_restore is not None,
            args.sync_delete is not None,
        ]
    )
    ...
    if (
        args.list
        or args.create is not None
        or args.restore is not None
        or args.delete is not None
    ) and not args.config:
