...
if command -v lspci &>/dev/null; then
    if lspci | grep -E "VGA|3D|Display" | grep -i "nvidia" &>/dev/null; then GPU_FOUND=true; fi
    if lspci | grep -E "VGA|3D|Display" | grep -iE "amd|radeon" &>/dev/null; then GPU_FOUND=true; fi
fi
...
start_daemon() {
    ...
    cd "$APP_DIR"
    env LD_LIBRARY_PATH="$CURRENT_LD" nohup uv run dusky_stt_main.py --daemon > "$DAEMON_LOG" 2>&1 &
    echo $! > "$PID_FILE"
...
if [[ -f "$RECORD_PID_FILE" ]] && kill -0 "$(cat "$RECORD_PID_FILE" 2>/dev/null)" 2>/dev/null; then
    RECORD_PID=$(cat "$RECORD_PID_FILE")
    kill -15 "$RECORD_PID" 2>/dev/null
    
    while kill -0 "$RECORD_PID" 2>/dev/null; do sleep 0.05; done
    ...
    if ! is_running; then
        start_daemon || { notify-send -u critical "STT Error" "Daemon startup failed"; exit 1; }
    fi
    ...
    notify-send -a "Parakeet STT" -u critical -t 2500 "Listening..." "Speak now. Trigger again to stop."
fi
EOF
